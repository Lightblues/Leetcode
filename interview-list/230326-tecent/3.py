""" 高维正态分布KL散度 [见 Tencent.md]
对于两个连续的正态分布, 它们KL散度 $D_{K L}(p \| q)=\int p(x) \log _{q(x)}^{p(x)} d x$
...
$$
D_{K L}(p \| q)={ }_2^1\left[\log \left|\Sigma_q\right| / \left|\Sigma_q\right| -k+\left(\boldsymbol{\mu}_p-\boldsymbol{\mu}_q\right)^T \Sigma_q^{-1}\left(\boldsymbol{\mu}_p-\boldsymbol{\mu}_q\right)+\operatorname{tr}\left\{\Sigma_q^{-1} \Sigma_p\right\}\right]
$$,
...
def f(sigma):
    # prod 求累乘
    # 应该还要 -k? 但不知道为啥加了就错了? 
    kl = 1/2 * (-math.log(abs(math.prod(sigma)))+ sum(sigma))
    return kl
for _ in range(q):
    k,t = map(float, input().split())
    sigma = list(map(float, input().split()))
    kl = f(sigma)
    print(1 if kl>t else 0)
